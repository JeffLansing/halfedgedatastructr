...
source("./R/half_edge_data_structure.R")
```
...
```{r required, echo=FALSE, out.width = '90%'}
knitr::include_graphics("man/figures/README-required.svg")
```
...
```{r provided, echo=FALSE, out.width = '95%'}
knitr::include_graphics("man/figures/README-provided.svg")
```
...
Generating spanning trees for polygons is a somewhat guarded art, and there doesn't seem to be code for that in R. Fortunately [SAGE](https://doc.sagemath.org/html/en/reference/graphs/sage/graphs/spanning_tree.html#sage.graphs.spanning_tree.spanning_trees) provides help for this. The following SAGE code (easily extended to other polygons) generates all (and only) the spanning trees for the tetrahedron:
...
```{r trees, echo=FALSE}
trees <- c('CF', 'CL', 'CM', 'CR', 'CX', 'CY', 'CU', 'C[', 'Cb', 'Ch', 'Ci', 'Cd', 'Ck', 'Cp', 'Cq', 'Cs')
knitr::kable(t(trees))
```
...
```
<!-- badges: start -->
[![R-CMD-check](https://github.com/JeffLansing/halfedgedatastructr/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/JeffLansing/halfedgedatastructr/actions/workflows/R-CMD-check.yaml)
<!-- badges: end -->
